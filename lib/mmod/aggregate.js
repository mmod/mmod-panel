...
mod.prototype.init = function()
{
    if( this.rig.settings.get_boolean( 'show-in-tray-menu' ) )
    {
        ...
        if( main.panel.statusArea.aggregateMenu !== undefined )
        {
            ...
            {
                main.panel.statusArea.aggregateMenu.menu._getMenuItems()[9].addMenuItem( new pum.PopupSeparatorMenuItem(), 1 );
                main.panel.statusArea.aggregateMenu.menu._getMenuItems()[9].addMenuItem( this.popup, 2 );
            }
            else
            {   // In which case we will target index 8 instead
                if( main.panel.statusArea.aggregateMenu.menu._getMenuItems()[8] )
                {
                    main.panel.statusArea.aggregateMenu.menu._getMenuItems()[8].addMenuItem( new pum.PopupSeparatorMenuItem(), 1 );
                    main.panel.statusArea.aggregateMenu.menu._getMenuItems()[8].addMenuItem( this.popup, 2 );
                }
            }
        }
        else
