    ...
    if( this.rig.settings.get_boolean( 'show-in-tray-menu' ) )
    {
        ...
        if( main.panel.statusArea.aggregateMenu !== undefined )
        {
            ...
            for( let i = 9; i >= 0; i-- )
            {
                ...
                {
                    if( typeof( main.panel.statusArea.aggregateMenu.menu.getMenuItems()[i].addMenuItem ) === typeof( function_model ) )
                    {
                    ...
                    {
                        console.log( '[MMOD Panel]: Could not find a point of entry to the Aggregate Menu.' );
                    }
                }
            }
        }
